        ...
        <div class="row">
                            ...
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu" role="menu">
                                    <a class="dropdown-item"
                                       href="{{ action('\PanicHD\PanicHD\Controllers\DashboardController@index', 2) }}">
                                        {{ trans('panichd::admin.index-3-months') }}
                                    </a>
                                    <a class="dropdown-item"
                                       href="{{ action('\PanicHD\PanicHD\Controllers\DashboardController@index', 5) }}">
                                        {{ trans('panichd::admin.index-6-months') }}
                                    </a>
                                    <a class="dropdown-item"
                                       href="{{ action('\PanicHD\PanicHD\Controllers\DashboardController@index', 11) }}">
            ...
            <div class="col-md-4">
                ...
                <div class="tab-content">
                    <ul id="information-panel-categories"
                        class="list-group tab-pane fade {{$active_tab == "cat" ? "show active" : ""}}" role="tabpanel">
                        ...
                        @foreach($categories as $category)
                            <li class="list-group-item d-flex">
                                <span style="color: {{ $category->color }}">{{ $category->name }} </span><span
                                        class="badge align-self-center ml-1 mr-auto"
                                        style="color: white; background-color: {{ $category->color }}">{{ $category->tickets()->count() }}</span>
                                ...
                                <span class="small">
                                    ...
                                    @if ($category->tickets()->newest()->count() == 0)
                                        {{ $a_button['newest'] }}
                                    @else
                                        <a href="{{ route($setting->grab('main_route') . '-filteronly', ['filter' => 'category', 'value' => $category->id, 'list' => 'newest']) }}"
                                           class="btn btn-light btn-xs"
                                           title="{{ trans('panichd::admin.index-view-category-tickets', ['list' =>trans('panichd::lang.newest-tickets-adjective'), 'category' => $category->name ]) }}">
										{{ $a_button['newest'] }}
										</a>
                                    @endif
									 -
									@if ($category->tickets()->active()->count() == 0)
                                        {{ $a_button['active'] }}
                                    @else
                                        <a href="{{ route($setting->grab('main_route') . '-filteronly', ['filter' => 'category', 'value' => $category->id, 'list' => 'active']) }}"
                                           class="btn btn-light btn-xs"
                                           title="{{ trans('panichd::admin.index-view-category-tickets', ['list' =>trans('panichd::lang.active-tickets-adjective'), 'category' => $category->name ]) }}">
										{{ $a_button['active'] }}
										</a>
                                    @endif
									 -
									@if ($category->tickets()->complete()->count() == 0)
                                        {{ $a_button['complete'] }}
                                    @else
                                        <a href="{{ route($setting->grab('main_route') . '-filteronly', ['filter' => 'category', 'value' => $category->id, 'list' => 'complete']) }}"
                                           class="btn btn-light btn-xs"
                                           title="{{ trans('panichd::admin.index-view-category-tickets', ['list' =>trans('panichd::lang.complete-tickets-adjective'), 'category' => $category->name ]) }}">
										{{ $a_button['complete'] }}
										</a>
                                    @endif
								</span>
                    ...
                    <ul id="information-panel-agents"
                              ...
                              @if ($setting->grab('user_route') != 'disabled')
                                ...
                                <span class="small">
                                    ...
                                    @if ($agent->ticketsAsAgent()->newest()->count() == 0)
                                        {{ $a_button['newest'] }}
                                    @else
                                        <a href="{{ route($setting->grab('main_route') . '-filteronly', ['filter' => 'agent', 'value' => $agent->id, 'list' => 'newest']) }}"
                                           class="btn btn-light btn-xs"
                                           title="{{ trans('panichd::admin.index-view-agent-tickets', ['list' =>trans('panichd::lang.newest-tickets-adjective')]) }}">
										{{ $a_button['newest'] }}
										</a>
                                    @endif
									 -
									@if ($agent->ticketsAsAgent()->active()->count() == 0)
                                        {{ $a_button['active'] }}
                                    @else
                                        <a href="{{ route($setting->grab('main_route') . '-filteronly', ['filter' => 'agent', 'value' => $agent->id, 'list' => 'active']) }}"
                                           class="btn btn-light btn-xs"
                                           title="{{ trans('panichd::admin.index-view-agent-tickets', ['list' =>trans('panichd::lang.active-tickets-adjective')]) }}">
										{{ $a_button['active'] }}
										</a>
                                    @endif
									 -
									@if ($agent->ticketsAsAgent()->complete()->count() == 0)
                                        {{ $a_button['complete'] }}
                                    @else
                                        <a href="{{ route($setting->grab('main_route') . '-filteronly', ['filter' => 'agent', 'value' => $agent->id, 'list' => 'complete']) }}"
                                           class="btn btn-light btn-xs"
                                           title="{{ trans('panichd::admin.index-view-agent-tickets', ['list' =>trans('panichd::lang.complete-tickets-adjective')]) }}">
										{{ $a_button['complete'] }}
										</a>
                                    @endif
                                </span>
                            ...
                            <li class="list-group-item d-flex">
                                ...
                                <span class="small">
                                    ...
                                    @if ($user->ticketsAsOwner()->newest()->count() == 0)
                                        {{ $a_button['newest'] }}
                                    @else
                                        <a href="{{ route($setting->grab('main_route') . '-filteronly', ['filter' => 'owner', 'value' => $user->id, 'list' => 'newest']) }}"
                                           class="btn btn-light btn-xs"
                                           title="{{ trans('panichd::admin.index-view-user-tickets', ['list' =>trans('panichd::lang.newest-tickets-adjective')]) }}">
										{{ $a_button['newest'] }}
										</a>
                                    @endif
									 -
									@if ($user->ticketsAsOwner()->active()->count() == 0)
                                        {{ $a_button['active'] }}
                                    @else
                                        <a href="{{ route($setting->grab('main_route') . '-filteronly', ['filter' => 'owner', 'value' => $user->id, 'list' => 'active']) }}"
                                           class="btn btn-light btn-xs"
                                           title="{{ trans('panichd::admin.index-view-user-tickets', ['list' =>trans('panichd::lang.active-tickets-adjective')]) }}">
										{{ $a_button['active'] }}
										</a>
                                    @endif
									 -
									@if ($user->ticketsAsOwner()->complete()->count() == 0)
                                        {{ $a_button['complete'] }}
                                    @else
                                        <a href="{{ route($setting->grab('main_route') . '-filteronly', ['filter' => 'owner', 'value' => $user->id, 'list' => 'complete']) }}"
                                           class="btn btn-light btn-xs"
                                           title="{{ trans('panichd::admin.index-view-user-tickets', ['list' =>trans('panichd::lang.complete-tickets-adjective')]) }}">
										{{ $a_button['complete'] }}
										</a>
                                    @endif
                                </span>
